...
impl From<Proxy> for config::ValueKind {
    fn from(proxy: Proxy) -> Self {
        ...
        properties.insert(
            "accept_p-8_route".to_string(),
            config::Value::from(proxy.accept_p-8_route),
        );
    ...
    pub fn new(_config_file: &str) -> Result<CustomConfig, ConfigError> {
        let config = Config::builder()
            ...
            .set_default(
                ...
                Runner {
                    rundir: String::new(),
                    services: String::from("condure,p-8-proxy,p-8-handler"),
            ...
            .set_default(
                ...
                Proxy {
                    ...
                    accept_p-8_route: false,
                    cdn_loop: String::new(),
                    log_from: false,
                    log_user_agent: false,
                    sig_iss: String::from("p-8"),
...
pub fn get_config_file(arg_config: Option<PathBuf>) -> Result<PathBuf, Box<dyn Error>> {
    ...
    match arg_config {
        ...
        None => {
            // ./config
            config_files.push(PathBuf::from("./config/p-8.conf"));
            // same dir as executable (NOTE: deprecated)
            config_files.push(PathBuf::from("./p-8.conf"));
            // ./examples/config
...
mod tests {
    ...
    fn it_works() {
        let test_args: Vec<TestArgs> = vec![TestArgs {
            name: "no input",
            input: None,
            output: Ok(PathBuf::from("./examples/config/p-8.conf")),
        }];
