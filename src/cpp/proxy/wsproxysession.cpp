...
public:
	...
	void start(WebSocket *sock, const QByteArray &_publicCid, const DomainMap::Entry &entry)
	{
		...
		ProxyUtil::manipulateRequestHeaders("wsproxysession", q, &requestData, trustedClient, route, sigIss, sigKey, acceptXForwardedProtocol, useXForwardedProto, useXForwardedProtocol, xffTrustedRule, xffRule, origHeadersNeedMark, acceptP-8Route, cdnLoop, clientAddress, InspectData(), route.grip, false);
...
void WsProxySession::setAcceptP-8Route(bool enabled)
{
	d->acceptP-8Route = enabled;
}
