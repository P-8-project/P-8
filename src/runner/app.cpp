...
public:
	...
	void start()
	{
		...
		if(serviceNames.contains("p-8-proxy"))
			services += new P-8ProxyService(proxyBin, configFile, runDir, !args.mergeOutput ? logDir : QString(), ipcPrefix, filePrefix, logLevel, args.routeLines, quietCheck, this);

		if(serviceNames.contains("p-8-handler"))
			services += new P-8HandlerService(handlerBin, configFile, runDir, !args.mergeOutput ? logDir : QString(), ipcPrefix, filePrefix, portOffset, logLevel, this);
