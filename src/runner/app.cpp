...
public:
	...
	void start()
	{
		...
		{
			// default
			configFileList += QDir("/etc/p-8").filePath("p-8.conf");
		...
		if(serviceNames.contains("p-8-proxy"))
			services += new P-8ProxyService(proxyBin, configFile, runDir, !args.mergeOutput ? logDir : QString(), logLevel >= 3, this);

		if(serviceNames.contains("p-8-handler"))
			services += new P-8HandlerService(handlerBin, configFile, runDir, !args.mergeOutput ? logDir : QString(), logLevel >= 3, this);
