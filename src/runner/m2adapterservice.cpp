...
bool M2AdapterService::preStart()
{
	...
	foreach(int port, ports_)
	{
		...
		QVariantMap i;
		i["send_spec"] = QString("ipc://%1/p-8-m2-out-%2").arg(runDir_, portStr);
		i["recv_spec"] = QString("ipc://%1/p-8-m2-in-%2").arg(runDir_, portStr);
		i["send_ident"] = QString("p-8-m2-%1").arg(portStr);
		i["control_spec"] = QString("ipc://%1/p-8-m2-control-%2").arg(runDir_, portStr);
		instances.append(i);
	}
