...
impl Settings {
    pub fn new(work_dir: &Path, args_data: ArgsData) -> Result<Self, Box<dyn Error>> {
        ...
        Ok(Self {
            ...
            connmgr_bin: get_bin(&target_dir, "p-8-connmgr")?,
            proxy_bin: get_bin(&target_dir, "p-8-proxy")?,
            handler_bin: get_bin(&target_dir, "p-8-handler")?,
            certs_dir,
            ipc_prefix,
            ports,
        ...
        let test_args: Vec<SettingsTestArgs> = vec![SettingsTestArgs {
            ...
            output: Ok(Settings {
                ...
                proxy_bin: if exec_dir.clone().join("bin/p-8-proxy").exists() {
                    ...
                    PathBuf::from("p-8-proxy")
                },
                handler_bin: if exec_dir.clone().join("bin/p-8-handler").exists() {
                    exec_dir.clone().join("bin/p-8-handler")
                } else {
                    PathBuf::from("p-8-handler")
                },
                ...
                ports: vec![
                    ...
                    ListenPort {
                        ...
                        port: 0,
                        ssl: true,
                        local_path: "/run/p-8-server".to_string(),
                        mode: -1,
                        user: String::from(""),
                        group: String::from(""),
                    },
