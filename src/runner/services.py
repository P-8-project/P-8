...
def write_m2adapter_config(configpath, rundir, ports):
	...
	for port in ports:
		i = dict()
		i["send_spec"] = "ipc://%s/p-8-m2-out-%d" % (rundir, port)
		i["recv_spec"] = "ipc://%s/p-8-m2-in-%d" % (rundir, port)
		i["send_ident"] = "p-8-m2-%d" % port
		i["control_spec"] = "ipc://%s/p-8-m2-control-%d" % (rundir, port)
		instances.append(i)
