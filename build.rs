...
fn read_conf_pri() -> Result<HashMap<String, String>, Box<dyn Error>> {
    ...
    for (k, v) in conf.iter_mut() {
        if SUFFIXED.contains(&k.as_str()) {
            match v.strip_suffix("/p-8") {
                Some(s) => *v = s.to_string(),
                None => return Err(format!("var {} missing /p-8 suffix", k).into()),
            }
        }
    }
...
fn write_postbuild_conf_pri(
    ...
    writeln!(&mut f, "LIBDIR = {}/p-8", lib_dir)?;
    writeln!(&mut f, "CONFIGDIR = {}/p-8", config_dir)?;
    writeln!(&mut f, "RUNDIR = {}/p-8", run_dir)?;
    writeln!(&mut f, "LOGDIR = {}/p-8", log_dir)?;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("cargo:rustc-env=CONFIG_DIR={}/p-8", config_dir);
    println!("cargo:rustc-env=LIB_DIR={}/p-8", lib_dir);
