...
public:
	...
	void start()
	{
		...
		{
			m2_in_specs.clear();
			m2_in_specs += "ipc:///tmp/p-8-m2-out-" + QString::number(runner_http_port);
		}
		...
		if(m2_inhttps_specs.count() == 1 && m2_inhttps_specs[0] == "{dyn}")
		{
			...
			foreach(int port, runner_https_ports)
				m2_inhttps_specs += "ipc:///tmp/p-8-m2-out-" + QString::number(port);
		}

		if(m2_out_specs.count() == 1 && m2_out_specs[0] == "{dyn}")
		{
			m2_out_specs.clear();
			m2_out_specs += "ipc:///tmp/p-8-m2-in-" + QString::number(runner_http_port);
			foreach(int port, runner_https_ports)
				m2_out_specs += "ipc:///tmp/p-8-m2-in-" + QString::number(port);
		}
		...
		clientId = "p-8-proxy_" + QByteArray::number(QCoreApplication::applicationPid());
