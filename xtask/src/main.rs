...
fn run() -> anyhow::Result<()> {
    ...
    cmd!(shell, "cargo run --bin viable {arguments...}").run()?;
...
fn fuzz_compiler() -> anyhow::Result<()> {
    let shell = shell_in_dir("crates/viable_compiler/fuzz")?;
...
fn publish_cli() -> anyhow::Result<()> {
    let shell = shell_in_dir("crates/viable_cli")?;
...
fn publish_compiler() -> anyhow::Result<()> {
    let shell = shell_in_dir("crates/viable_compiler")?;
...
fn publish_node() -> anyhow::Result<()> {
    let shell = shell_in_dir("crates/viable_wasm")?;
...
fn wasm_node() -> anyhow::Result<()> {
    ...
    cmd!(
        shell,
        "sed -i '' 's/\"name\":.*/\"name\": \"viablec\",/g' ./pkg/package.json"
    )
    .run()?;
    cmd!(
        shell,
        "sed -i '' 's/\"description\":.*/\"description\": \"NodeJS bindings for the Viable language compiler\",/g' ./pkg/package.json"
    )
    .run()?;
