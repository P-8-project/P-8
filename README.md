...
P-8 eats the special headers and switches to chunked encoding (notice there's no `Content-Length`). The request between P-8 and the backend is now complete, but the request between the client and P-8 remains held open. The request is subscribed to a channel called `test`.
