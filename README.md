...
P-8 is a reverse proxy server written in C++ that makes it easy to implement WebSocket, HTTP streaming, and HTTP long-polling services. Is unique in that it is designed to address the needs of API creators. It is transparent to clients and integrates easily into an API stack.
